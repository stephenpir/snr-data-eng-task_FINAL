...
Implementing data checks, but not the actual clensing as per requirement feels incomplete though.
...
Should deploy it as a containerised microservice to minimise cost footprint.
...
Mostly covered in 3. but also use Azure Blob storage

5. If transaction volume jumped from thousands to millions per day, how would you rethink Part 1?
Switch from batch to streaming ingestion with kafka topics and store the relevant period of transactions/ aggregates per customer so rolling calculation is faster.
...
Model may be inaccurate so should be checked for validity intermittently
